...
// Import Hotel room images
import hotelRoom1 from "@/assets/gallery/hotel-room-1.jpg";
import hotelRoom2 from "@/assets/gallery/hotel-room-2.jpg";
import hotelRoom3 from "@/assets/gallery/hotel-room-3.jpg";
import hotelRoom4 from "@/assets/gallery/hotel-room-4.jpg";
...
import birdwatchVirginEarthImg from "@/assets/birdwatch-virgin-earth-enhanced.jpg";
...
const hotelRoomImages = [hotelRoom1, hotelRoom2, hotelRoom3, hotelRoom4];
...
const CategoryPage = () => {
  ...
  const isHotelBookings = category.id === "hotels";
  ...
  return (
    <div className="min-h-screen bg-background animate-page-enter">
      ...
      {isHotelBookings && (
        <section className="py-12 px-4">
          <div className="container mx-auto">
            ...
            {/* Booking CTA */}
            <div className="bg-gradient-to-r from-primary/10 via-secondary/10 to-primary/10 rounded-3xl p-8 md:p-12 text-center border border-primary/20">
              <div className="text-5xl mb-4">🏨</div>
              <h3 className="text-2xl md:text-3xl font-serif font-bold text-foreground mb-4">
                We Will Book Rooms <span className="text-gradient-primary">For You</span>
              </h3>
              <p className="text-muted-foreground text-lg mb-8 max-w-2xl mx-auto">
                Let us handle your accommodation! We partner with the best hotels and resorts in Ooty & Nilgiris to offer you premium stays at the best prices.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
                ...
                <a 
                  href={`https://wa.me/${phoneNumber}?text=${encodeURIComponent("Hi Raghul, I'm interested in hotel bookings in Ooty. Please share available options.")}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1"
                >
                  <Button size="xl" className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white">
      ...
      {/* Tour Packages - Only for non-toy-train and non-hotel categories */}
      {!isToyTrain && !isHotelBookings && category.packages.length > 0 && (
        <section className="py-12 px-4">
          <div className="container mx-auto">
            <div className="mb-10">
              <h2 className="text-2xl md:text-3xl font-serif font-bold text-foreground mb-3">
              ...
              {category.packages.map((pkg) => (
                <div key={pkg.id} className="group bg-card rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border border-border flex flex-col md:flex-row">
                  <div className="relative w-full md:w-[400px] h-56 md:h-auto shrink-0 overflow-hidden">
                    ...
                    {!isPickupDrop && pkg.price && <div className="absolute top-4 left-4 px-3 py-1.5 bg-primary text-primary-foreground rounded-full text-sm font-bold">{pkg.price}</div>}
                  </div>
